...
- `tasks/PROGRESS.md`는 작업 상태의 단일 기준이다. 모든 작업은 시작 시 목표·범위를, 진행 중 범위 변경 또는 차단 시 그 사유를, 완료 시 변경 범위·검증 결과·미해결 항목을 같은 작업 단위에서 갱신한다.
...
- `tasks/PROGRESS.md`에 이번 작업의 상태와 검증·차단 결과를 갱신했는가?
...
- 작업 시작 시 `tasks/PROGRESS.md`에 목표와 범위를 기록하고, 범위 변경·차단이 생길 때 즉시 갱신한다. 각 회차가 끝나면 완료 범위, 검증 결과, 미해결 항목과 환경 제약을 반드시 갱신한다.
